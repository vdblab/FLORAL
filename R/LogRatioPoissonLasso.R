LogRatioPoissonLasso <- function(x,
                                 y,
                                 ncov,
                                 length.lambda=100,
  ...
  # Score of the intercept-only Poisson model (log link): mu = mean(y)
  sfun = y - mean(y)
  ...
  # Cross-validated Poisson deviance for one column of linear predictors
  pois.dev <- function(eta,yt){
    mufit <- exp(pmin(pmax(eta,-30),30))
    term <- yt*log(yt/mufit)
    term[yt==0] <- 0
    2*sum(term - (yt - mufit))/length(yt)
  }

  if (progress) cat("Algorithm running for full dataset: \n")

  fullfit <- poisson_enet_al(x,y,length.lambda,mu,maxiter,lambda,wcov,a,adjust,ncov,progress,loop1,loop2)
  ...
  if (!is.null(ncv)){
    ...
    if (is.null(foldid)){
      labels <- caret::createFolds(seq_along(y),k=ncv,list=FALSE)
    }else{
      labels <- foldid
    }
    ...
    if (ncore == 1){
      ...
      for (cv in 1:ncv){
        ...
        cvfit <- poisson_enet_al(train.x,train.y,length.lambda,mu,maxiter,lambda,wcov,a,adjust,ncov,progress,loop1,loop2)

        cvmse[,cv] <- apply(cbind(1,test.x) %*% rbind(t(cvfit$beta0),cvfit$beta),2,function(eta) pois.dev(eta,test.y))
      ...
      cvmse <- foreach(cv=1:ncv,.combine=cbind) %dopar% {
        ...
        cvfit <- poisson_enet_al(train.x,train.y,length.lambda,mu,maxiter,lambda,wcov,a,adjust,ncov,FALSE,loop1,loop2)

        apply(cbind(1,test.x) %*% rbind(t(cvfit$beta0),cvfit$beta),2,function(eta) pois.dev(eta,test.y))
    ...
    if (plot){
      ...
      pmse <- ggplot(mseplot, aes(x=.data$loglambda, y=.data$mse)) +
        ...
        ggtitle(expression(paste("Cross-validated deviance versus log(",lambda,")")))
      ...
      if (!adjust){
        ...
        if (length(which(ret$best.beta$min.mse!=0)) > 1){
          ...
          if (ncol(x.select.min) > 1){
            stepglmnet <- cv.glmnet(x=x.select.min,y=y,type.measure = "deviance",family="poisson")
          ...
          step2fit <- suppressWarnings(step(glm(y~.,data=df_step2,family=poisson),trace=0))
