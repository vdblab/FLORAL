LogRatioLassoKN <- function(x,
  ...
  if (a > 0){
    lambda0 <- max(abs(t(y) %*% cbind(x,xk)))/(a*n)
  }else if (a == 0){
    lambda0 <- max(abs(t(y) %*% cbind(x,xk)))/(1e-3*n)
  }
  ...
  fullfit <- linear_enet_al_kn(cbind(x,xk),y,length.lambda,mu,maxiter,lambda,wcov,a,adjust,ncov,progress)
  ...
  if (intercept){
    beta0 <- mean(y0) - colMeans(cbind(x,xk) %*% fullfit$beta)
  }
  
  df_betaseq <- data.frame(t(fullfit$beta[(ncov+1):p,]),check.names = FALSE)
