...
To make this vignette run quickly, we could subset to a fraction of the original samples:
```{r}
set.seed(12543)
...
fit <- FLORAL(x = x, y = y$study_condition_int, family="gaussian", ncv=10, progress=TRUE)
```
...
fit$step2.tables$`1se` |> data.frame() |> dplyr::filter(Pr...t.. < .05) |> arrange(desc(abs(Estimate)))
# or fit$step2.tables$min
