...
Summarizing `FLORAL` outputs from multiple random k-fold cross
validations
...
- plot:
  ...
  `TRUE` or `FALSE`, indicating whether returning summary plots of
  selection probability for taxa features.

## Value

A list with relative frequencies of a certain feature being selected
over `mcv` `ncv`-fold cross-validations.
